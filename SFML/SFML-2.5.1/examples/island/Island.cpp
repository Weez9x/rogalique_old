...
float getElevation(float x, float y)
{
    ...
    for (int i = 0; i < perlinOctaves; i++)
    {
        elevation += stb_perlin_noise3(x * perlinFrequency * std::pow(perlinFrequencyBase, i),
                                       y * perlinFrequency * std::pow(perlinFrequencyBase, i), 0, 0, 0, 0) *
...
{
    sf::Color color = moisture < 0.27f ? sf::Color(240, 240, 180)
                      : moisture < 0.3f
                          ? sf::Color(240 - 240 * (moisture - 0.27f) / 0.03f, 240 - 40 * (moisture - 0.27f) / 0.03f,
...
sf::Vector2f computeNormal(int x, int y, float left, float right, float bottom, float top)
{
    ...
    sf::Vector3f crossProduct(deltaX.y * deltaY.z - deltaX.z * deltaY.y, deltaX.z * deltaY.x - deltaX.x * deltaY.z,
        ...
        for (int x = 0; x < resolutionX; x++)
        {
            ...
            else
            {
                ...
                vertices[arrayIndexBase + 0].texCoords =
                    computeNormal(x, y, getElevation(x - 1, y), getElevation(x + 1, y), getElevation(x, y + 1),
                                  getElevation(x, y - 1));
            }
            ...
            else
            {
                ...
                vertices[arrayIndexBase + 1].texCoords =
                    computeNormal(x, y + 1, getElevation(x - 1, y + 1), getElevation(x + 1, y + 1),
            ...
            vertices[arrayIndexBase + 2].texCoords =
                computeNormal(x + 1, y + 1, getElevation(x, y + 1), getElevation(x + 2, y + 1),
            ...
            else
            {
                ...
                vertices[arrayIndexBase + 5].texCoords =
                    computeNormal(x + 1, y, getElevation(x, y), getElevation(x + 2, y), getElevation(x + 1, y + 1),
