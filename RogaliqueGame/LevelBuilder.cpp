...
namespace RogaliqueGame
{
	void LevelBuilder::BuildLevel()
	{
		...
		for (int row = 0; row < rows; ++row)
		{
			for (int col = 0; col < cols; ++col)
			{
				...
				floorTiles.push_back(
					std::make_unique<FloorTile>(
						EngineGame::Vector2Df((float)x, (float)y),
						tileSize,
						tileSize,
						0
					)
		...
		for (int col = 0; col < cols; ++col)
		{
			...
			walls.push_back(
				std::make_unique<Wall>(
					EngineGame::Vector2Df((float)x, (float)top),
					tileSize,
					tileSize
				)
			);
			...
			walls.push_back(
				std::make_unique<Wall>(
					EngineGame::Vector2Df((float)x, (float)bottom),
					tileSize,
					tileSize
				)
			);
		...
		for (int row = 1; row < rows - 1; ++row)
		{
			...
			walls.push_back(
				std::make_unique<Wall>(
					EngineGame::Vector2Df((float)left, (float)y),
					tileSize,
					tileSize
				)
			);
			...
			walls.push_back(
				std::make_unique<Wall>(
					EngineGame::Vector2Df((float)right, (float)y),
					tileSize,
					tileSize
				)
			);
